...
void ThumbnailBuilder::RebuildCache(AssetItem &_root)
{
    ...
    AX_LOG_EDITOR_INFO("Finished rebuilding thumbnail cache in : {1:d} seconds.",
...
void AssetManager::reload()
{
    ...
    root = AssetItem(AssetItem::Type::DIRECTORY, std::string{"Catagories"}, assetsPath);
    discoverAssets(root);
    auto end = std::chrono::steady_clock::now();
    AX_LOG_EDITOR_INFO("Finished reloading assets in : {1:d} seconds.",
                       std::chrono::duration_cast<std::chrono::seconds>(end - start).count())
}
    ...
    for (auto &p : std::filesystem::directory_iterator(parent.path))
    {
        if (p.is_directory())
        {
            parent.children.push_back(AssetItem(AssetItem::Type::DIRECTORY,
                                                p.path().filename().string(), p.path().string()));
        ...
        else if (p.is_regular_file())
        {
            ...
            else if (extension.compare(".jpeg") | extension.compare(".png") | extension.compare(".jpg") | extension.compare(".tif") | extension.compare(".tiff"))
            {
            ...
            {
                AX_LOG_EDITOR_WARN("Unkown asset file type with extension {}.", extension)
            }
        }
    }
}