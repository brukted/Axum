...
void PackageManager::LoadPackage(std::string &Path)
{
    ...
    AX_LOG_EDITOR_INFO("Loaded package {0} in : {1:d} milliseconds", mPackages.back().name.GetValue(), std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count())
}
...
void PackageManager::SavePackage(Package &pkg)
{
    ...
    oa << pkg;
    auto end = std::chrono::steady_clock::now();
    AX_LOG_EDITOR_INFO("Saved package {0} in : {1:d} milliseconds", mPackages.back().name.GetValue(), std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count())
}
