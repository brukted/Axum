 ...
 */
MaterialOutSocket::MaterialOutSocket(std::string &_name, unsigned int _uid, Channel _channel) : channel(_channel), OutputSocket(_name, _uid) {}

MaterialOutSocket::MaterialOutSocket(const char *_name, unsigned int _uid, Channel _channel) : channel(_channel), OutputSocket(_name, _uid) {}
...
void MaterialOutSocket::SetupCache()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GetGlFormat((ImageFormat)parent.imageFormat.GetValue()), parent.width.GetValue(), parent.height.GetValue(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
